...
def input_with_validation(prompt:str, type: str, can_be_empty=False, lower_bound=None, upper_bound=None):
    """
            ...
            type : str
                The type of variable that will be return by the function ['integer','float' or 'string']
        ...
        try:
            ...
            if not lower_bound is None and not upper_bound is None:
                if choice_cast >= lower_bound and choice_cast <= upper_bound:
                    ...
                    print("Wrong choice. Please provide an answer between {low} and {up}".format(low=lower_bound,up=upper_bound))
                    choice_cast = None
                    choice=input()
...
def main():
    InitForm = namedtuple("InitForm", ["user", "id" ])
